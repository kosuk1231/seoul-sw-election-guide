 ...
 import { guDistricts, seoulGus } from "@/data/districts";
 
 export default function CandidatesGu() {
   const [searchTerm, setSearchTerm] = useState("");
   const [selectedGu, setSelectedGu] = useState<string>("all");
 
   const filteredDistricts = useMemo(() => {
     return guDistricts.filter((district) => {
       const matchesSearch =
       ...
       <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
         ...
         <div className="mb-8">
           <div className="flex items-center gap-3 mb-4">
             <div className="flex h-12 w-12 items-center justify-center rounded-xl bg-civic-sage text-primary-foreground">
               <Building2 className="h-6 w-6" />
             </div>
             <div>
               <h1 className="text-2xl sm:text-3xl font-bold text-foreground">구의원 선거구</h1>
               <p className="text-muted-foreground">자치구의회 의원 선거구 현황</p>
             </div>
           </div>
         </div>
           ...
           {Object.entries(groupedDistricts).map(([guName, districts]) => (
             <div key={guName}>
               <h2 className="text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                 <MapPin className="h-4 w-4 text-civic-sage" />
                 {guName}
